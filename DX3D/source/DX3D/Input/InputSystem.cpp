...
dx3_d::InputSystem::InputSystem(const InputSystemDesc& desc) : Base(desc.base)
{
}

bool dx3_d::InputSystem::isKeyDown(KeyCode key) const
{
	return m_currentKeys[static_cast<std::size_t>(key)];
}

bool dx3_d::InputSystem::isKeyPressed(KeyCode key) const
{
	return m_currentKeys[static_cast<std::size_t>(key)] &&
		!m_previousKeys[static_cast<std::size_t>(key)];
}

bool dx3_d::InputSystem::isKeyReleased(KeyCode key) const
{
	return !m_currentKeys[static_cast<std::size_t>(key)] &&
		m_previousKeys[static_cast<std::size_t>(key)];
}
...
dx3_d::Vec2 dx3_d::InputSystem::getMousePosition() const noexcept
{
	return m_mousePosition;
}

dx3_d::Vec2 dx3_d::InputSystem::getMouseDelta() const noexcept
{
	return m_mouseDelta;
}


void dx3_d::InputSystem::setCursorVisible(bool visible)
{
...
void dx3_d::InputSystem::setCursorLocked(bool locked)
{
	m_cursorLocked = locked;
	if (locked) centerCursor();
}

void dx3_d::InputSystem::setCursorLockArea(const Rect& rect)
{
	m_lockArea = rect;
}

void dx3_d::InputSystem::centerCursor()
{
...
short dx3_d::InputSystem::getInternalKeyCode(const KeyCode& key)
{
